...
token = '6324369008:AAEfJEjHJW2_VozhQUdQLuixXJ-2X4ulzgs'
sudo = 6376489778
s = requests.session()
...
def start(message):
    ...
    donefilee = types.InlineKeyboardButton(text='Send Done Users 🧝🏻‍♂️', callback_data='done')
...
def sidinput(message):
    ...
    sidd = types.InlineKeyboardButton(text='Set Session ⚓️', callback_data='sid')
...
def check(message):
    ...
    markup_inline = types.InlineKeyboardMarkup()
    sid = types.InlineKeyboardButton(text='Set Session ⚓️', callback_data='sid')
    global RunPosts
    RunPosts = True
                    ...
                    while RunPosts == True:
                        for checkusers in user:
                            ...
                            if rem == 0:
                                ...
                                RunPosts = False
                            if not RunPosts:
                                bot.send_message(message.chat.id, text='𝗜 𝗔𝗠 𝗦𝗧𝗢𝗣 🧜🏻‍♂️')
                                s.cookies.clear()
                                s.close()
                                break
                            ...
                            if 'This username isn’t available.' in res.text or '"is_valid":false' in res.text or '"is_valid":true' in res.text:
                                if os.path.exists('sessionid.txt'):
                                    ...
                                    bot.send_message(message.chat.id, text='Call @xx_YaBh')
                                    break
                            elif 'Login expired' in res.text:
                                bot.send_message(message.chat.id,
                                                 text='🍕 𝗬𝗼𝘂𝗿 𝘀𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗗 𝗵𝗮𝘀 𝗯𝗲𝗲𝗻 𝗲𝘅𝗽𝗶𝗿𝗲𝗱',
...
def done(message):
    ...
    if file.exists():
        ...
        li = len(list.readlines())
        if li == 0:
            bot.send_message(message.chat.id, text='𝗙𝗜𝗟𝗘, 𝗜𝗦 𝗘𝗠𝗣𝗧𝗬 𝗦𝗜𝗥 🎟️')
        else:
...
def make(message, amount):
    ...
    if os.path.exists("list.txt"):
        ...
        bot.send_message(message.chat.id, text='𝗜 𝗔𝗠 𝗗𝗢𝗡𝗘 𝗦𝗜𝗥 🛎️', reply_markup=markup_inline)
